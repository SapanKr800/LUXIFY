...
const ProductCard = ({ product }) => {
    ...
    return (
        <>
            <div className='group relative hover-lift cursor-pointer block bg-[#181717] fade-up' key={product.id}
            ...
            >
                <div className="relative aspect-3/4 overflow-hidden bg-card mb-4 shadow-sm rounded-none">
                    ...
                    <div className={`absolute bottom-4 left-4 right-4 flex gap-2 transition-all duration-300 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} `}>
                        ...
                        <button
                            ...
                            className="flex-1 py-3 bg-[#ffffff] text-[#000000] text-sm tracking-wider hover:bg-[#D9A520] hover:text-[#000000!important]
                             transition-colors rounded-sm flex items-center justify-center gap-2"
                        >
                            <ShoppingBag className="w-4 h-4" /> {isInCart ? "Remove from Bag" : "Add to Bag"}
                        </button>
                    </div>
                ...
                <div className="space-y-1 p-4">
                    <p className="font-body text-xs tracking-wider text-[#A39E8F] uppercase">{product.category}</p>
                    <h3 className="font1 text-lg  text-[#F4F2EB]"> {product.name}</h3>
                    <p className="font-body text-sm text-[#D9A520]">${product.price}</p>
                </div>
